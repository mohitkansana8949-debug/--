...
import { Loader, Youtube, UserCircle, Calendar, Video } from 'lucide-react';
...
export default function LiveClassesPage() {
    ...
    const isLoading = upcomingLoading || recordedLoading;
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {recordedClasses?.map(liveClass => (
                        <ClassCard key={liveClass.id} liveClass={liveClass} isRecorded />
                    ))}
                </div>
            </div>
        </div>
...
function ClassCard({ liveClass, isRecorded = false }: { liveClass: any, isRecorded?: boolean }) {
     return (
            ...
            <CardFooter>
                <Button asChild className="w-full">
                    <Link href={`/live-classes/${liveClass.id}`}>
                        {isRecorded ? 'वीडियो देखें' : 'क्लास ज्वाइन करें'}
                    </Link>
                </Button>
            </CardFooter>
        </Card>
